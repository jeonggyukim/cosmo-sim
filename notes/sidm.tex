...
This section is a survey of the method landscape.  It covers the
phenomenon rather than the practice of simulating it: the DSMC
algorithm in full, the cost of a three-dimensional generalisation,
moment-closure schemes and asymptotic-preserving integrators are
developed in a separate note.
...
application.  Each arrow in the chain is either exact or rests on a
stated approximation, so the failure reported by
\citet{Gurian2025Core} can be attributed to one particular arrow rather
than to the framework as a whole: the Liouville, BBGKY and Boltzmann
steps remain valid in an SIDM halo, and it is the final
Chapman--Enskog step --- which assumes the distribution stays close to
Maxwellian and the mean free path stays short compared with the
gravitational scale height --- that breaks in the
intermediate-mean-free-path shell (\S\ref{app:CE_breakdown}).
Presentation follows the pedagogical order of van den Bosch's ASTR 501
lecture notes (\emph{Dynamics of Astrophysical Many-Body Systems}).

\subsection{Hamiltonian $N$-body and Liouville's theorem}
\label{app:liouville}

This subsection summarises what is needed and no more.  The same
material --- the Legendre transform, where Hamilton's equations come
from, Poisson brackets, canonical transformations, and the numerical
consequences of Liouville's theorem --- is developed at length in
\texttt{classical\_mechanics.tex}.

\paragraph{Phase space and the Hamiltonian flow.}  Consider a system of
$N$ particles that interact with one another.  To specify the state of
this system completely at one instant, we must give the position and
the momentum of every single particle.  It is convenient to collect all
of these numbers into one long list,
%
\begin{equation}
  \vec\Gamma = (\vec q_1,\ldots,\vec q_N,\vec p_1,\ldots,\vec p_N),
  \label{eq:gamma_point}
\end{equation}
%
which has $6N$ components, since each of the $N$ particles contributes
three position components and three momentum components.  The space of
all possible values of $\vec\Gamma$ is called \emph{$\Gamma$-space}.  A
single point of $\Gamma$-space represents the complete state of the
entire $N$-particle system.  This should not be confused with the
$6$-dimensional space of one particle's own position and momentum,
which is called \emph{$\mu$-space} and which will appear in
\S\ref{app:bbgky}.

The system evolves according to Hamilton's equations,
%
\begin{equation}
  \dot{\vec q}_i = \frac{\partial\mathcal H}{\partial\vec p_i},
  \qquad
  \dot{\vec p}_i = -\frac{\partial\mathcal H}{\partial\vec q_i},
  \qquad i = 1,\ldots,N,
  \label{eq:hamilton}
\end{equation}
%
where $\mathcal H(\vec\Gamma)$ is the Hamiltonian of the system.  These
are $6N$ coupled first-order differential equations.  It is useful to
read them in the following way.  At every point of $\Gamma$-space, the
right-hand sides of \eqref{eq:hamilton} are definite numbers, and
together they form a $6N$-component vector
%
\begin{equation}
  \dot{\vec\Gamma} = (\dot{\vec q}_1,\ldots,\dot{\vec q}_N,
                      \dot{\vec p}_1,\ldots,\dot{\vec p}_N).
  \label{eq:gamma_velocity}
\end{equation}
%
In other words, Hamilton's equations attach a velocity to every point
of $\Gamma$-space.  A rule of this kind is called a \emph{velocity
field}, and the motion it generates is called a \emph{flow}.

Two properties of this flow will be used repeatedly.  First, the
velocity at a given point depends only on the location of that point,
and not on how the system arrived there or at what time.  Second, it
follows that a trajectory is determined completely by its starting
point.  Two different trajectories can therefore never pass through the
same point of $\Gamma$-space, because that point would have to carry
two different velocities at once.  Trajectories in $\Gamma$-space
neither cross nor merge.

\paragraph{Liouville's theorem.}  Imagine that $\Gamma$-space is filled
with an ordinary fluid, and that this fluid moves with the velocity
field $\dot{\vec\Gamma}$ defined in \eqref{eq:gamma_velocity}.  A
natural question is whether such a fluid can be compressed.  In
ordinary fluid mechanics the answer is given by the divergence of the
velocity field: a positive divergence means that the fluid is expanding
locally, a negative divergence that it is being squeezed, and a
vanishing divergence that its volume is preserved.  Let us therefore
compute the divergence of $\dot{\vec\Gamma}$.  Since $\Gamma$-space has
$6N$ dimensions, the divergence contains $6N$ terms, which we group
into position and momentum pairs:
%
\begin{equation}
  \nabla_{\!\vec\Gamma}\!\cdot\!\dot{\vec\Gamma}
   = \sum_{i=1}^{N}\!\left(
       \frac{\partial}{\partial\vec q_i}\!\cdot\!\dot{\vec q}_i
     + \frac{\partial}{\partial\vec p_i}\!\cdot\!\dot{\vec p}_i\right)
   = \sum_{i=1}^{N}\!\left(
       \frac{\partial^2\mathcal H}{\partial\vec q_i\,\partial\vec p_i}
     - \frac{\partial^2\mathcal H}{\partial\vec p_i\,\partial\vec q_i}\right)
   = 0.
  \label{eq:liouville_theorem}
\end{equation}
%
The second equality is obtained by substituting Hamilton's equations
\eqref{eq:hamilton} for $\dot{\vec q}_i$ and $\dot{\vec p}_i$.  The
third equality holds because the order in which two partial derivatives
are taken does not matter, so the two second derivatives of
$\mathcal H$ are equal and cancel.  The cancellation happens
separately for each particle $i$, and it relies on the minus sign in
the second of Hamilton's equations.

The result \eqref{eq:liouville_theorem} is \emph{Liouville's theorem}.
Stated in words: the flow generated by Hamilton's equations is
incompressible.  A more useful way to state it is in terms of volumes.
Choose any region $R_0$ of $\Gamma$-space at time $t=0$, and let every
point of that region move along the flow.  At a later time $t$ the
points occupy some new region $R_t$.  Liouville's theorem says that
$R_t$ and $R_0$ have the same $6N$-dimensional volume.  The shape of
the region may become extremely distorted, so that a compact blob is
drawn out into a thin filament wrapped many times around the accessible
part of $\Gamma$-space, but the volume enclosed is unchanged.

A simple case makes this concrete.  For a one-dimensional harmonic
oscillator, $\mathcal H = p^2/2m + \tfrac12 m\omega^2 q^2$, Hamilton's
equations give $\dot q = p/m$ and $\dot p = -m\omega^2 q$.  The
trajectories are ellipses in the $(q,p)$ plane, whose axes depend on
$m$ and $\omega$, so it is easier to see what is happening after the
change of variables
%
\begin{equation}
  Q \equiv \sqrt{m\omega}\;q,
  \qquad
  P \equiv p/\!\sqrt{m\omega}.
  \label{eq:sho_canonical}
\end{equation}
%
This is a \emph{canonical} transformation, meaning that Hamilton's
equations keep their form in the new variables.\footnote{The two scale
factors must be reciprocals of each other.  A rescaling
$Q = \lambda q$, $P = \mu p$ is canonical only when $\lambda\mu = 1$,
which \eqref{eq:sho_canonical} satisfies with
$\lambda = \sqrt{m\omega}$ and $\mu = 1/\!\sqrt{m\omega}$.  Scaling the
momentum alone, that is $Q = q$ and $P = p/m\omega$, gives
$\lambda\mu = 1/m\omega \neq 1$; that transformation is not canonical,
and Hamilton's equations do not hold in the variables it produces.}
The condition
for this is that the transformation preserve phase-space area, and
\eqref{eq:sho_canonical} does, since its Jacobian is
%
\begin{equation}
  \frac{\partial(Q,P)}{\partial(q,p)}
   = \sqrt{m\omega}\,\cdot\,\frac{1}{\sqrt{m\omega}} = 1 :
\end{equation}
%
the factor $\sqrt{m\omega}$ multiplying the coordinate cancels against
the factor $1/\!\sqrt{m\omega}$ multiplying the momentum.  Area
preservation is the right criterion here because there is only one
degree of freedom; for more than one it is necessary but not
sufficient, and the general test is that the new variables reproduce
the Poisson brackets \eqref{eq:poisson_bracket} of the old ones.  See
\texttt{classical\_mechanics.tex}~\S7.  Both new variables carry the
same dimensions,
and the Hamiltonian becomes
%
\begin{equation}
  \mathcal H = \tfrac12\,\omega\bigl(Q^2 + P^2\bigr),
\end{equation}
%
whose level sets are circles rather than ellipses.  Hamilton's
equations now read
$\dot Q = \partial\mathcal H/\partial P = \omega P$ and
$\dot P = -\partial\mathcal H/\partial Q = -\omega Q$, which describe a
rigid rotation of the $(Q,P)$ plane at angular velocity $\omega$.  A
rotation preserves area, and the change of variables preserved area as
well, so area is preserved in the original $(q,p)$ plane too --- in
agreement with the theorem.

One point deserves emphasis before we continue.  The derivation of
\eqref{eq:liouville_theorem} used Hamilton's equations, and used
nothing else.  No probability, no statistical assumption, and no
distribution function entered anywhere.  Liouville's theorem is a
geometric property of Hamiltonian dynamics itself.

\paragraph{The Liouville equation.}  In practice we almost never know
the exact microstate of a many-particle system.  The standard way to
handle this ignorance is to consider an \emph{ensemble}: a large
collection of copies of the system, all prepared so as to satisfy the
same macroscopic conditions, but differing from one another in their
microscopic details.  Each copy is one point of $\Gamma$-space, so the
ensemble is a cloud of points.  We describe this cloud by its density,
the $N$-body distribution function $f^{(N)}(\vec\Gamma,t)$, normalised
so that
%
\begin{equation}
  \int f^{(N)}(\vec\Gamma,t)\;d^{6N}\vec\Gamma = 1 .
  \label{eq:fN_norm}
\end{equation}
%
With this normalisation, $f^{(N)}\,d^{6N}\vec\Gamma$ is the probability
that the true microstate of the system lies inside the small volume
$d^{6N}\vec\Gamma$.  Our goal is an equation of motion for $f^{(N)}$.

\paragraph{A caution about the word ``cloud''.}  It should not be taken
to suggest that the ensemble members lie close to one another.  They do
not.  Two microstates with identical macroscopic properties are
typically enormous distances apart in $\Gamma$-space: exchanging the
momenta of two molecules moves the representative point a long way
while changing nothing measurable, and there are $N!$ such
relabellings.  Fixing a few macroscopic numbers --- energy, particle
number, volume --- leaves a surface of dimension close to $6N$, so the
ensemble occupies essentially the whole region of $\Gamma$-space that
the constraints permit, at very low density, rather than a compact
blob.

What makes the word ``density'' meaningful is therefore not proximity
but measure.  Equation \eqref{eq:fN_norm} defines $f^{(N)}$ through the
probability assigned to a small volume, and that is well posed however
thinly and however widely the probability is spread.  Nothing in the
derivation below requires the ensemble to be localised or even to
occupy a connected region: the theorem
\eqref{eq:liouville_theorem} and the continuity equation we are about
to write down are both statements at each point of $\Gamma$-space
separately.

Two further points are worth making explicit, since the fluid analogy
can mislead in both.  First, the ensemble members do not interact.
Each copy of the system evolves on its own under the same
$\mathcal H$, and no copy exerts any influence on another; the fluid
language is borrowed only because probability is conserved and carried
by a velocity field, which are the two ingredients a continuity
equation needs.  Second, a genuinely finite ensemble of $M$ copies
would give $M$ isolated points rather than a smooth function, so
$f^{(N)}$ is the idealisation reached as $M\to\infty$.  Some authors
avoid the ensemble altogether and read $f^{(N)}$ as a description of
what the observer knows about the system.  The equation derived below
is the same in either reading.

As time passes, every point of the cloud moves along the flow.  No copy
of the system is created and none is destroyed, and we showed above
that trajectories neither cross nor merge.  The number of points is
therefore conserved, and $f^{(N)}$ obeys a continuity equation of
exactly the same form as the one satisfied by the mass density of an
ordinary fluid, $\partial\rho/\partial t + \nabla\!\cdot\!(\rho\vec u)
= 0$.  Here the fluid velocity is $\dot{\vec\Gamma}$ and the space is
$\Gamma$-space:
%
\begin{equation}
  \frac{\partial f^{(N)}}{\partial t}
   + \nabla_{\!\vec\Gamma}\!\cdot\!\bigl(f^{(N)}\dot{\vec\Gamma}\bigr) = 0 .
  \label{eq:gamma_continuity}
\end{equation}
%
Expanding the divergence with the product rule turns this into three
terms,
%
\begin{equation}
  \frac{\partial f^{(N)}}{\partial t}
   + \dot{\vec\Gamma}\!\cdot\!\nabla_{\!\vec\Gamma} f^{(N)}
   + f^{(N)}\,\bigl(\nabla_{\!\vec\Gamma}\!\cdot\!\dot{\vec\Gamma}\bigr)
   = 0 ,
  \label{eq:gamma_continuity_expanded}
\end{equation}
%
each of which has a clear meaning.  The first term is the change of
density at a fixed location in $\Gamma$-space.  The second describes
density being carried past that location by the flow.  The third
describes density changing because the flow itself is compressing or
expanding the fluid.

It is precisely this third term that Liouville's theorem removes.  By
\eqref{eq:liouville_theorem} the factor
$\nabla_{\!\vec\Gamma}\!\cdot\!\dot{\vec\Gamma}$ vanishes identically,
so the compression term is absent.  This is the only place in the
derivation where the theorem is needed, and the step should not be
passed over quickly.  Conservation of probability by itself gives only
\eqref{eq:gamma_continuity}, in which the density at a co-moving point
would still change whenever the flow squeezed or stretched its
surroundings.  Only because the flow is incompressible does a conserved
density become a density that stays constant along the flow.

It remains to write the second term of
\eqref{eq:gamma_continuity_expanded} in standard Hamiltonian notation.
For any two functions $A$ and $B$ defined on phase space, the
\emph{Poisson bracket} is
%
\begin{equation}
...
\end{equation}
%
Writing the second term of \eqref{eq:gamma_continuity_expanded} out
component by component and then substituting Hamilton's equations
\eqref{eq:hamilton} gives
%
\begin{equation}
  \dot{\vec\Gamma}\!\cdot\!\nabla_{\!\vec\Gamma}f^{(N)}
   = \sum_{i=1}^{N}\!\left(
       \dot{\vec q}_i\!\cdot\!\frac{\partial f^{(N)}}{\partial\vec q_i}
     + \dot{\vec p}_i\!\cdot\!\frac{\partial f^{(N)}}{\partial\vec p_i}\right)
   = \{f^{(N)},\mathcal H\},
  \label{eq:advective_is_bracket}
\end{equation}
%
which is the Poisson bracket \eqref{eq:poisson_bracket} with $A =
f^{(N)}$ and $B = \mathcal H$.  Collecting the surviving two terms, we
obtain the \emph{Liouville equation}
%
\begin{equation}
...
\end{equation}
%
The two forms given here say the same thing, and passing from the first
to the second requires only the chain rule, not any additional physics.
To see this, follow one ensemble member along its trajectory
$\vec\Gamma(t)$ and ask how the density in its immediate neighbourhood
changes.  The chain rule gives
$df^{(N)}/dt = \partial f^{(N)}/\partial t
+ \dot{\vec\Gamma}\!\cdot\!\nabla_{\!\vec\Gamma}f^{(N)}$, and by
\eqref{eq:advective_is_bracket} the second piece is the Poisson
bracket.  The physical content of $df^{(N)}/dt = 0$ is then easy to
state: an observer who drifts along with the ensemble sees the
phase-space density in their neighbourhood remain constant forever.

Finally, a warning about terminology.  Two different statements have
been derived above, and both are commonly called after Liouville.
Equation \eqref{eq:liouville_theorem}, \emph{Liouville's theorem}, is a
property of the Hamiltonian flow; it was proved from Hamilton's
equations alone, before any distribution function was introduced.
Equation \eqref{eq:liouville}, the \emph{Liouville equation}, is the
evolution law for $f^{(N)}$; it follows once that geometric property is
combined with the conservation of probability.  Keeping the two apart
makes the logic of the following sections much easier to follow.

\subsection{From Liouville to Boltzmann: the BBGKY hierarchy}
\label{app:bbgky}

The Liouville equation \eqref{eq:liouville} is exact, but it is also
unusable.  Its unknown $f^{(N)}$ is a function of $6N$ variables, and
for a dark-matter halo $N$ may be of order $10^{69}$.  No calculation
and no computer can handle such an object.  Fortunately we never need
it.  The quantities we actually measure --- a density profile, a
velocity dispersion, a scattering rate --- involve only one or two
particles at a time.  This section constructs the objects that describe
one or two particles, and derives the equations they obey.

\paragraph{Reducing the number of variables.}  The way to describe a
few particles instead of all of them is to integrate the others away.
Write $\vec w_i \equiv (\vec q_i,\vec p_i)$ for the position and
momentum of particle $i$ taken together, so that a single $\vec w_i$ is
a point of the $6$-dimensional $\mu$-space introduced in
\S\ref{app:liouville}.  We then define the \emph{reduced $k$-particle
...
\end{equation}
%
Two separate operations appear here, and it is worth naming them
individually.  The integral is an ordinary marginalisation: starting
from a joint probability distribution over $N$ variables, we integrate
over the $N-k$ variables we are not interested in and keep a
distribution over the remaining $k$.  This is a standard operation in
probability theory and involves no physics and no approximation.  The
prefactor $N!/(N-k)!$ is a counting factor, and its role is explained
next.

\paragraph{Why the counting factor is there.}  Recall from
\eqref{eq:fN_norm} that $f^{(N)}$ is normalised to unity.  Recall also
that the particles are identical, which means that $f^{(N)}$ is
unchanged if any two of its arguments are exchanged.  Applying
\eqref{eq:reduced_df} with $k=1$ and $k=2$ and using the normalisation
of $f^{(N)}$ then gives
%
\begin{equation}
  \int f^{(1)}\,d^6\vec w_1 = N,
  \qquad
  \int f^{(2)}\,d^6\vec w_1\,d^6\vec w_2 = N(N-1).
  \label{eq:reduced_df_norm}
\end{equation}
%
These results show what the prefactor accomplishes.  Without it, the
marginal distribution would integrate to one and would answer the
question ``what is the probability that the particle we have labelled
number~$1$ is found here?''  That question is not useful, because the
labels are arbitrary: the particles are identical and no physical
particle carries the label $1$.  With the prefactor, $f^{(1)}$
integrates to $N$ and answers a question we can actually ask, namely
``how many particles, on average, are found here?''  The factor
$N!/(N-k)!$ is exactly the number of ordered ways of choosing $k$ of
the $N$ identical particles to occupy the $k$ tracked slots.

Consequently $f^{(1)}$ is not a probability density but a \emph{number}
density in $\mu$-space.  It is the same distribution function used
throughout stellar dynamics, and the familiar spatial quantities follow
from it by integrating over momenta,
%
\begin{equation}
  n(\vec x) = \!\int\! f^{(1)}(\vec x,\vec p)\,d^3\vec p,
  \qquad
  \rho(\vec x) = m\,n(\vec x).
  \label{eq:n_from_f1}
\end{equation}
%
The choice also has a technical benefit that will become visible in
Step~C of the derivation below: with this normalisation, the equations
of the hierarchy contain no explicit factors of $N$.  Textbooks that
normalise every $f^{(k)}$ to unity instead must carry factors of
$(N-k)$ through each subsequent step.

\paragraph{What the reduced functions describe.}  It helps to state
plainly what each member of the family means.  The function
$f^{(1)}(\vec x,\vec p)$ gives the average number of particles per unit
volume of $\mu$-space, so $f^{(1)}d^3\vec x\,d^3\vec p$ is the expected
number of particles found in a small cell at $(\vec x,\vec p)$.  The
function $f^{(2)}(\vec w_1,\vec w_2)$ gives the expected number of
\emph{ordered pairs} of particles with one member in a cell at
$\vec w_1$ and the other in a cell at $\vec w_2$.  Similarly $f^{(3)}$
counts triplets, and so on.  Each $f^{(k)}$ is therefore a $k$-point
counting density.

Readers who work on large-scale structure will recognise the second of
these.  Integrating $f^{(2)}$ over both momenta leaves a purely spatial
pair count, which is conventionally written in terms of the two-point
correlation function $\xi$:
%
\begin{equation}
  n^{(2)}(\vec x_1,\vec x_2) \equiv \!\int\! f^{(2)}\,d^3\vec p_1\,d^3\vec p_2
    = \bar n^{\,2}\bigl[1 + \xi(|\vec x_1 - \vec x_2|)\bigr].
  \label{eq:f2_xi}
\end{equation}
%
So $\xi(r)$, measured from galaxy catalogues or from the particles of a
simulation, is nothing other than a momentum-integrated version of
$f^{(2)}$.  The reduced distribution functions are the phase-space
generalisation of the $n$-point statistics of cosmology.

\paragraph{Why one or two particles are enough.}  A more practical
justification for \eqref{eq:reduced_df} is that the reduced functions
already determine every quantity we are likely to want.  Suppose an
observable is a sum over single particles,
$A_{\rm tot} = \sum_{i} A(\vec w_i)$, as is the case for the density,
the mean velocity, the pressure and the kinetic energy.  Using the
symmetry of $f^{(N)}$ and the definition \eqref{eq:reduced_df}, its
ensemble average is
%
\begin{equation}
  \bigl\langle A_{\rm tot}\bigr\rangle
   = \!\int\! A(\vec w)\,f^{(1)}(\vec w)\;d^6\vec w ,
  \label{eq:onebody_average}
\end{equation}
%
which requires $f^{(1)}$ and nothing more.  Suppose instead that the
observable is a sum over pairs, as the potential energy and the
collision rate both are.  Then
%
\begin{equation}
  \Bigl\langle \tfrac12\!\sum_{i\neq j} U(|\vec q_i-\vec q_j|)\Bigr\rangle
   = \frac12\!\int\! U(|\vec q_1-\vec q_2|)\,
       f^{(2)}(\vec w_1,\vec w_2)\;d^6\vec w_1\,d^6\vec w_2 ,
  \label{eq:twobody_average}
\end{equation}
%
which requires $f^{(2)}$ and nothing more.  Discarding the information
in $f^{(3)}$ and above therefore costs us nothing that we were going to
measure.  This is the real reason the reduction is useful, rather than
merely convenient.

\paragraph{Mayer cluster expansion.}  Part of the information carried
by $f^{(2)}$ is already contained in $f^{(1)}$.  If two particles were
statistically independent, the expected number of pairs would simply be
the product of the expected numbers of single particles.  It is
therefore natural to split off that trivial part and give a name to the
remainder.  Writing $f\equiv f^{(1)}$ and labelling the arguments by
number for brevity,
%
\begin{align}
...
\end{align}
%
The function $g(1,2)$ measures genuine two-body correlation, and
$h(1,2,3)$ measures the irreducible three-body correlation that remains
after every pair correlation has been subtracted.  If the particles are
statistically independent then $g$ and $h$ both vanish, and
\eqref{eq:f2_xi} then gives $\xi = 0$, as it should.  The functions $g$
and $h$ are the phase-space counterpart of the connected correlation
functions of cosmology.  Every closure introduced below will consist of
setting one of them to zero, so it is worth being clear at this stage
about what they mean.

One small qualification is needed for a system with a fixed number of
particles.  Comparing the two normalisations in
\eqref{eq:reduced_df_norm} with the corresponding integral of the
product $f(1)f(2)$, which is $N^2$, we find
%
\begin{equation}
  \int g(1,2)\;d^6\vec w_1\,d^6\vec w_2 = N(N-1) - N^2 = -N .
  \label{eq:g_self_term}
\end{equation}
%
So $g$ cannot vanish identically when $N$ is finite.  The residual
piece expresses only the fact that a particle cannot form a pair with
itself, and it is smaller than the leading term by a factor $1/N$.  It
carries no dynamical information.  It is, however, the same $1/N$ that
appears later when we estimate the size of gravitational correlations
as $g\sim ff/N$.

\paragraph{Derivation of the hierarchy.}  We now ask what equation
$f^{(k)}$ obeys.  The strategy is simple to describe: differentiate the
definition \eqref{eq:reduced_df} with respect to time, replace
$\partial f^{(N)}/\partial t$ using the Liouville equation, and carry
out as many of the integrals as possible.  Everything that follows is
exact.

To make the interactions explicit, split the Hamiltonian into one-body
and pair terms,
%
\begin{equation}
  \mathcal H = \sum_{i=1}^{N} \frac{\vec p_i^{\,2}}{2m}
             + \sum_{i=1}^{N} \Phi_{\rm ext}(\vec q_i)
             + \frac12\sum_{i\neq j} U(|\vec q_i - \vec q_j|),
  \label{eq:H_split}
\end{equation}
%
in which $\Phi_{\rm ext}$ is any external potential and
$U(|\vec q_i-\vec q_j|)$ is the interaction between a pair of
particles.  The factor $\tfrac12$ prevents each pair from being counted
twice.  With this Hamiltonian, and with Hamilton's equations
substituted as in \eqref{eq:advective_is_bracket}, the Liouville
equation \eqref{eq:liouville} reads
%
\begin{equation}
  \frac{\partial f^{(N)}}{\partial t}
   = -\sum_{j=1}^{N}\!\left[
       \frac{\vec p_j}{m}\!\cdot\!\frac{\partial f^{(N)}}{\partial\vec q_j}
     + \vec F_j\!\cdot\!\frac{\partial f^{(N)}}{\partial\vec p_j}\right],
  \qquad
  \vec F_j \equiv -\frac{\partial\mathcal H}{\partial\vec q_j}
     = -\nabla\Phi_{\rm ext}(\vec q_j)
       - \sum_{l\neq j}\frac{\partial U(|\vec q_j - \vec q_l|)}{\partial\vec q_j}.
  \label{eq:liouville_expanded}
\end{equation}
%
Here $\vec F_j$ is the total force acting on particle $j$.  We now
apply the operator $\bigl[N!/(N-k)!\bigr]\int\prod_{i>k}d^6\vec w_i$ to
both sides of \eqref{eq:liouville_expanded}, which by definition turns
the left-hand side into $\partial f^{(k)}/\partial t$.  On the
right-hand side, the sum over $j$ runs over all $N$ particles, and the
particles of the two kinds behave very differently.  We therefore split
the sum into the $k$ tracked particles ($j\le k$) and the $N-k$
integrated-out particles ($j>k$), and treat the two groups in turn.

\emph{Step A: the integrated-out particles contribute nothing
directly.}  Take a term with $j>k$.  Both of its pieces are derivatives
with respect to a variable that is itself being integrated over, so
each can be converted into a boundary term.  For the streaming piece,
the integral over $\vec q_j$ of a gradient is a surface integral by the
divergence theorem,
%
\begin{equation}
  \int\! d^3\vec q_j\;\frac{\vec p_j}{m}\!\cdot\!\frac{\partial f^{(N)}}{\partial\vec q_j}
   = \frac{\vec p_j}{m}\!\cdot\!\oint\! d\vec S\; f^{(N)} = 0,
\end{equation}
%
and this vanishes because $f^{(N)}\to 0$ far away in position space.
If the system is instead placed in a periodic box, the contributions
from opposite faces cancel and the conclusion is the same.  For the
force piece, note first that $\vec F_j$ depends on positions only and
therefore commutes with $\partial/\partial\vec p_j$.  This lets us
rewrite the piece as a divergence in momentum space, which again
becomes a boundary term,
%
\begin{equation}
  \int\! d^3\vec p_j\;\vec F_j\!\cdot\!\frac{\partial f^{(N)}}{\partial\vec p_j}
   = \int\! d^3\vec p_j\;\frac{\partial}{\partial\vec p_j}\!\cdot\!
     \bigl(\vec F_j\, f^{(N)}\bigr) = 0,
\end{equation}
%
which vanishes because $f^{(N)}\to 0$ as $|\vec p_j|\to\infty$: a state
of arbitrarily large energy has no probability of occurring.

Every term with $j>k$ therefore disappears.  The physical statement
behind this piece of algebra is worth extracting.  The untracked
particles move and accelerate, but in doing so they conserve their own
probability, so their motion integrates away to nothing.  What survives
of them is not their own dynamics but their influence on the particles
we are tracking, and that influence enters only through the force
$\vec F_j$ with $j\le k$.  That is the subject of the next two steps.

\emph{Step B: the terms that close among the tracked particles.}  Now
take a term with $j\le k$.  Its streaming piece involves
$\partial/\partial\vec q_j$, and $\vec q_j$ is not an integration
variable, so this derivative passes through the integral unchanged and
produces $(\vec p_j/m)\cdot\partial f^{(k)}/\partial\vec q_j$.  The
force piece needs more care, because $\vec F_j$ contains contributions
from every other particle in the system, tracked and untracked alike.
We split it according to which particle is exerting the force:
%
\begin{equation}
  \vec F_j =
    \underbrace{-\nabla\Phi_{\rm ext}(\vec q_j)
      - \sum_{\substack{l\le k\\ l\neq j}}
        \frac{\partial U(|\vec q_j-\vec q_l|)}{\partial\vec q_j}}
      _{\text{internal to the tracked subsystem}}
    \;\;
    \underbrace{-\sum_{l>k}
        \frac{\partial U(|\vec q_j-\vec q_l|)}{\partial\vec q_j}}
      _{\text{external: will require } f^{(k+1)}}.
  \label{eq:force_split}
\end{equation}
%
The first group depends only on the tracked coordinates
$\vec q_1,\ldots,\vec q_k$.  Like the streaming piece, it therefore
passes through the integral unchanged.  Combining it with the streaming
piece and comparing with the definition \eqref{eq:poisson_bracket} of
the Poisson bracket, we find that these terms assemble into
$-\{f^{(k)},\mathcal H^{(k)}\}$, which equals
$+\{\mathcal H^{(k)},f^{(k)}\}$ because the bracket changes sign when
its arguments are exchanged.  The Hamiltonian appearing here is the one
belonging to the tracked particles alone,
%
\begin{equation}
  \mathcal H^{(k)} = \sum_{i\le k}\!\left[\frac{\vec p_i^{\,2}}{2m}
     + \Phi_{\rm ext}(\vec q_i)\right]
     + \frac12\!\!\sum_{\substack{i,j\le k\\ i\neq j}}\!\! U(|\vec q_i-\vec q_j|).
  \label{eq:H_k}
\end{equation}
%
In short, this group of terms describes the tracked particles as though
they formed an isolated system of their own.  Note that for $k=1$ the
interaction term in \eqref{eq:H_k} is absent altogether, since a single
particle does not interact with itself; in that case
$\{\mathcal H^{(1)},f^{(1)}\}$ describes free streaming in the external
potential and nothing else.

\emph{Step C: the coupling to the untracked particles.}  There remains
the second group of \eqref{eq:force_split}, the force exerted on a
tracked particle $j$ by the untracked particles $l>k$.  Since these
$\vec q_l$ \emph{are} integration variables, this group cannot be moved
outside the integral, and it is here that the untracked particles make
their appearance.  Two minus signs are involved, one from
\eqref{eq:liouville_expanded} and one from the definition of
$\vec F_j$, so the group enters with an overall positive sign:
%
\begin{equation}
  +\frac{N!}{(N-k)!}\sum_{j\le k}\;\sum_{l>k}\!\int\!\prod_{i>k}d^6\vec w_i\;
    \frac{\partial U(|\vec q_j-\vec q_l|)}{\partial\vec q_j}
    \cdot\frac{\partial f^{(N)}}{\partial\vec p_j}.
\end{equation}
%
At first sight this expression is intractable, because it still refers
to all $N-k$ untracked particles.  It simplifies because the labels
$l = k+1,\ldots,N$ are dummy integration variables, and because
$f^{(N)}$ is unchanged when they are permuted.  Every term of the sum
over $l$ therefore evaluates to the same number.  We may consequently
keep just one representative term, the one with $l = k+1$, and multiply
it by the number of terms, which is $N-k$.  The counting factor in
front then combines with this multiplicity,
%
\begin{equation}
  (N-k)\,\frac{N!}{(N-k)!} = \frac{N!}{(N-k-1)!},
  \label{eq:prefactor_collapse}
\end{equation}
%
which is exactly the prefactor that \eqref{eq:reduced_df} assigns to
$f^{(k+1)}$.  Combined with the integral that remains over
$\vec w_{k+2},\ldots,\vec w_N$, this reconstitutes the next member of
the sequence,
%
\begin{equation}
  \frac{N!}{(N-k-1)!}\!\int\!\prod_{i=k+2}^{N} d^6\vec w_i\; f^{(N)}
    = f^{(k+1)}(\vec w_1,\ldots,\vec w_{k+1}).
\end{equation}
%
One last rearrangement is permitted: the derivative
$\partial/\partial\vec p_j$ may be taken outside the integral over
$\vec w_{k+1}$, since the two act on independent variables and
$\partial U/\partial\vec q_j$ does not depend on $\vec p_j$.  Collecting
the results of Steps~A, B and~C, we arrive at the \emph{BBGKY
hierarchy}, named after Bogoliubov, Born, Green, Kirkwood and Yvon:
%
\begin{equation}
...
\end{equation}
%
Observe that no factor of $N$ survives anywhere in \eqref{eq:bbgky}.
This is the benefit of the counting factor promised earlier: it was
consumed exactly by the multiplicity $N-k$ in
\eqref{eq:prefactor_collapse}.

\paragraph{A check on the counting ($N=2$).}  It is easy to lose a
factor in a derivation of this kind, so it is worth testing the result
on the smallest non-trivial system.  Take $N=2$.  For $k=1$ the
counting factor is $N!/(N-1)! = 2$, and for $k=2$ it is
$N!/(N-2)! = 2$; the multiplicity appearing in
\eqref{eq:prefactor_collapse} is $(N-k)\,N!/(N-k)! = 1\times 2 = 2$,
which agrees with the $f^{(2)}$ normalisation as required.  Now look at
the $k=2$ member of \eqref{eq:bbgky}.  Its coupling term sums over
$l>2$, and with $N=2$ there is no such particle, so the sum is empty.
The equation reduces to
$\partial_t f^{(2)} = \{\mathcal H^{(2)},f^{(2)}\}$, which is simply
the Liouville equation for a two-particle system.  The hierarchy closes
at the top, exactly as it should.

\paragraph{How to read the hierarchy.}  Equation \eqref{eq:bbgky} has
two terms, and each has a clear interpretation.  The Poisson bracket is
the dynamics the tracked particles would obey if they were an isolated
system: streaming, the external potential, and their mutual
interactions.  The integral is the correction due to everything else in
the system.  It represents the momentum kick delivered to a tracked
particle by one further particle located at
$(\vec q_{k+1},\vec p_{k+1})$, and it is weighted by the probability of
finding that particle there \emph{given} where the tracked particles
already are.  Within the integrand, the factor
$\partial U/\partial\vec q_i$ supplies the strength of the force, and
$\partial f^{(k+1)}/\partial\vec p_i$ describes the resulting
displacement of probability in momentum space.

The important structural feature is that the equation for $f^{(k)}$
contains $f^{(k+1)}$.  One-particle dynamics requires knowledge of
pairs; pair dynamics requires triplets; and so on.  We therefore do not
have a single equation but a chain of $N$ coupled equations, which
closes only at $k=N$, where the coupling term is empty and
\eqref{eq:bbgky} becomes the Liouville equation once more.  Nothing has
been assumed anywhere in the derivation except Hamiltonian dynamics, so
no information has been gained and none has been lost: BBGKY is a
rewriting of the Liouville equation in a more convenient set of
variables.  It follows that all of the physics of a kinetic theory
enters at the point where this chain is cut.

Since the case of interest is almost always $k=1$, it is useful to
write that member out in full:
%
\begin{equation}
  \frac{\partial f}{\partial t}
   + \frac{\vec p_1}{m}\!\cdot\!\frac{\partial f}{\partial\vec q_1}
   - \nabla\Phi_{\rm ext}\!\cdot\!\frac{\partial f}{\partial\vec p_1}
   = \!\int\! d^3\vec q_2\,d^3\vec p_2\;
     \frac{\partial U(|\vec q_1-\vec q_2|)}{\partial\vec q_1}
     \cdot\frac{\partial f^{(2)}(\vec w_1,\vec w_2)}{\partial\vec p_1},
  \label{eq:bbgky1}
\end{equation}
%
The left-hand side is familiar and harmless; the entire difficulty of
kinetic theory sits in the integral on the right, which we cannot
evaluate without knowing $f^{(2)}$.  Each of the theories constructed
below corresponds to one particular assumption about what to write
there.

\paragraph{Closure I: neglecting correlations entirely
(Vlasov--Poisson).}  The simplest possible assumption is that the
particles are statistically independent, so that the correlation
function $g$ of \eqref{eq:mayer2} vanishes at every separation and
$f^{(2)}(1,2) = f(1)f(2)$.  Substituting this into
\eqref{eq:bbgky1}, and noting that $f(\vec w_2)$ does not depend on
$\vec p_1$ and so may be moved past the momentum derivative, the
integral factorises into two independent pieces,
%
\begin{equation}
  \!\int\! d^3\vec q_2\,d^3\vec p_2\;
    \frac{\partial U}{\partial\vec q_1}\!\cdot\!
    \frac{\partial f^{(2)}}{\partial\vec p_1}
  = \underbrace{\left[\int\! d^3\vec q_2\,d^3\vec p_2\;
      \frac{\partial U(|\vec q_1-\vec q_2|)}{\partial\vec q_1}\,
      f(\vec w_2)\right]}_{\textstyle \nabla\Phi(\vec q_1)}
    \!\cdot\!\frac{\partial f(\vec w_1)}{\partial\vec p_1},
  \label{eq:vlasov_factorisation}
\end{equation}
%
The quantity in the bracket deserves attention.  It no longer depends
on $\vec p_1$, nor on the state of any individual second particle; it
depends only on the position $\vec q_1$.  It is therefore an ordinary
force field, produced by the smoothed-out distribution of all the other
particles.  In the gravitational case it is the familiar mean-field
force $\nabla\Phi$, with $\Phi$ obtained from the density
$\rho = m\!\int f\,d^3\vec p$ through Poisson's equation.  Being a
force field, it belongs on the left-hand side of the equation alongside
the streaming term, and once it is moved there the $k=1$ equation no
longer refers to $f^{(2)}$ at all.  The chain is cut, and what remains
is the \emph{collisionless Boltzmann equation}, also called the CBE or
the Vlasov equation:
%
\begin{equation}
  \frac{\partial f}{\partial t}
...
\end{equation}
%
This equation governs collisionless stellar dynamics, and it is what a
pure cold dark matter $N$-body simulation solves.  The assumption
behind it is a strong one, since $g$ was set to zero at all separations
and not merely where two particles happen to touch.  For long-range
forces such as gravity it is nevertheless the appropriate assumption.
There is no small region in which the interaction is concentrated, so
no natural place to make an exception; and once $g(1,2)=0$ has been
imposed, collisionless dynamics preserves it exactly.

\paragraph{Closure II: neglecting correlations only at contact
(the Boltzmann equation).}  Consider now the opposite situation, a
dilute gas whose particles interact only at short range, so that $U$ is
non-zero only inside a small collision volume of radius
$r_{\rm coll}$.  Closure~I is of no use here.  The mean field built in
\eqref{eq:vlasov_factorisation} from such an interaction is negligibly
small, because the interaction has almost no spatial reach, and yet the
individual encounters between particles are certainly not negligible:
they are the entire physics of the gas.

What is needed is a weaker assumption, one that discards correlations
only where they obstruct the calculation.  We therefore keep $g(1,2)$
in general, and set it to zero only at the moment of contact.  In other
words, we assume that the momenta of two particles \emph{about to
collide} are uncorrelated,
%
\begin{equation}
  f^{(2)}(\vec x,\vec x,\vec p_1,\vec p_2)
   = f^{(1)}(\vec x,\vec p_1)\,f^{(1)}(\vec x,\vec p_2),
  \label{eq:stosszahlansatz}
\end{equation}
%
where the two position arguments coincide because the interaction acts
only when the particles are at the same place.  This assumption is
known as \emph{molecular chaos}, or by its German name the
\emph{Stosszahlansatz}, literally the ``collision-number assumption''.
With it, the $k=1$ equation of \eqref{eq:bbgky} again closes on itself,
this time as the \emph{Boltzmann equation}
%
\begin{equation}
  \frac{df}{dt}
...
\end{equation}
%
Here $\omega$ is the differential cross section for the scattering
process, and $f$ denotes $f^{(1)}$ throughout.  The two terms in the
square bracket have opposite roles.  The first counts collisions whose
outcome places a particle at momentum $\vec p_1$, and so adds to $f$;
it is called the gain term.  The second counts collisions that remove a
particle from $\vec p_1$, and so subtracts from $f$; it is the loss
term.

One consequence of \eqref{eq:stosszahlansatz} should be noted, since it
is the origin of a well-known puzzle.  The assumption is imposed on the
particles \emph{entering} a collision, but not on those leaving it,
which are of course correlated by the encounter they have just had.
This treatment of the two directions of time is not symmetric, and it
is what allows an irreversible equation to emerge from mechanics that
is perfectly reversible.  The difficulty is known as the Loschmidt
paradox.  It is a genuine conceptual issue, and it is set aside here
for the usual reason: the resulting equation describes real gases
extremely well.

\paragraph{Where SIDM stands.}  Self-interacting dark matter is
governed by both types of interaction at once.  Its particles attract
one another gravitationally at long range, which is the situation
Closure~I was designed for, and they also scatter elastically at short
range, which is the situation Closure~II was designed for.  The
appropriate phase-space equation therefore borrows from both.  It is
the Boltzmann equation
\eqref{eq:boltzmann}--\eqref{eq:collision_integral}, with the potential
$\Phi$ determined by Poisson's equation as in \eqref{eq:cbe}: the
collisionless dynamics of a CDM simulation, with a collision term added
on the right.  Setting $I[f]\to 0$ recovers pure CDM, and this is the
sense in which SIDM lies between the two closures.

\paragraph{Checking the closures with numbers.}  The two closures
differ only in \emph{where} the correlation $g(1,2)$ is set to zero:
everywhere in the first case, and only at contact in the second.
Whether either is justified in a given system is not a matter of
preference but of magnitude, and it is worth putting numbers to it.

Consider first the gravitational part of $U$.  Correlations between
particles interacting through a long-range force are of relative size
$g\sim ff/N$, the same $1/N$ that appeared in \eqref{eq:g_self_term}.
A halo of mass $10^{12}\Msun$ composed of particles of mass
$m_\chi = 1$~GeV contains roughly $N\sim10^{69}$ of them.  The
correlations are thus smaller than the mean-field term by some seventy
orders of magnitude, the corresponding two-body relaxation time is far
longer than the age of the Universe, and Closure~I is for practical
purposes exact.  The situation is different in a simulation, where the
same halo is represented by only $N\sim10^{6}$ to $10^{9}$ particles.
There the neglected correlations are not small, and spurious two-body
relaxation is a real numerical effect; softening the gravitational
force at small separations is the standard way of suppressing it.

Consider next the scattering part of $U$, for which the relevant
quantity is the dilution $n\,r_{\rm coll}^3$, the number of particles
inside one collision volume.  Take the benchmark cross section
$\smx = 1\cmg$ with $m_\chi = 1$~GeV, a density typical of a dwarf
galaxy, $\rho = 0.1\Msun\,\mathrm{pc^{-3}}$, and a relative velocity
$\vrel = 30\kms$.  Then
%
\begin{align}
  \Gamma &= (\smx)\,\rho\,\vrel \approx 2\times10^{-17}\ \mathrm{s^{-1}},
    &\Gamma^{-1} &\approx 1.6\ \mathrm{Gyr},
    \label{eq:sidm_rate_estimate}\\
  \mfp &= \vrel/\Gamma \approx 50\ \mathrm{kpc} \gg r_{\rm halo},
    &n &= \rho/m_\chi \approx 4\ \mathrm{cm^{-3}},\\
  r_{\rm coll} &\sim (\sigma/4\pi)^{1/2} \approx 4\times10^{-13}\ \mathrm{cm},
    &n\,r_{\rm coll}^3 &\sim 10^{-37}.
\end{align}
%
The last of these numbers shows that the assumption of molecular chaos
is satisfied with an enormous margin to spare: a collision volume
contains essentially no other particles, so there is no mechanism by
which the colliding pair could have been correlated beforehand.  The
first number is equally informative.  A scattering time comparable to
the age of the Universe is not an accident but the defining property of
the SIDM benchmark.  Collisions must be rare enough that any
correlation created by one encounter has decayed long before the next,
which is what justifies Closure~II, and frequent enough to transport a
significant amount of heat over the lifetime of a halo, which is what
makes the model observationally interesting in the first place.

The mean free path deserves one further comment.  Being far larger than
the halo itself, it places the system in what is called the
long-mean-free-path regime, and this will cause difficulties later,
when we attempt to describe the halo as a fluid in
\S\ref{app:CE_breakdown}.  Those difficulties belong to a much later
stage of the derivation chain.  The BBGKY hierarchy \eqref{eq:bbgky} is
exact, and the Boltzmann equation \eqref{eq:boltzmann} is accurate to
the degree indicated by the numbers above; nothing that goes wrong
further downstream reflects on either of them.
