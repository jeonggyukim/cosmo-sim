...
itself does not need to be strongly coupled.
...
$d\log M_{\rm core}/d\log\langle v^2\rangle$ exponent differs from the
fluid value by an order of magnitude in the SMFP regime, so the seed
mass at relativistic instability could be substantially larger than
fluid extrapolations predict.  Most of the halo's lifetime is still
...
mass grid.  Codes: SoulICE (Essig+2019), gravothermal.jl (Pollack+2015
lineage), \texttt{RecfastJulia}-style pipelines.  Limitations:
...
%
\begin{itemize}
  \item The velocity distribution is bimodal-anisotropic: cold
        particles from the core mix with hot particles from outside,
        neither fully thermalising.
  ...
  \item Ni entropy-production rate $\hat Q$ (which vanishes in local
        equilibrium) turns positive.
\end{itemize}
%
