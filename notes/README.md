...
Some notes that once lived here have been removed: the dark-matter physics notes
(`sidm`, `fdm`, `dm_chemistry`, `qft`, `classical_mechanics`) on 2026-07-28, and
`cosmo_stat` on 2026-09-03. One criterion decided each: a note stays only while
it depends on this repository's pipeline for its runs, its measured numbers or
its figures. All six that remain do.

`cosmo_ic` and `xi_estimators` still cite `cosmo_stat.pdf` for background. It is
companion reading, not something they build against, and both stand without it.
