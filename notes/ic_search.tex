...
conditions used here that surprised a collaborator, and that anyone measuring
...
per cent. That is not a defect of the transfer function code, and switching
between CLASS and a tabulated CAMB file does not change it. It follows from
back-scaling.

With back-scaling, the default, CLASS is evaluated at the back-scaling target
redshift, $z_{\rm target} = 0$, and the field is scaled to the starting redshift
with the growth factor. By $z = 0$ the baryons have long since fallen into the
...
up, and makes the back-scaling factor unity. Measured at $N = 256^3$,
...
at $z = 0$; the price is that the split between the components at the starting
redshift is not the physical one. The forward field is the true field at the
starting redshift, with the physical split, and carries no such guarantee about
$z = 0$.
...
matters for anyone comparing the two components to each other, which is the
situation that raised the question.
