...
sharpest feature in $P(k)$ is the one cosmologists most want to measure.
This subsection identifies the feature, works out how much of it survives
the convolution, and gives the numbers for one specific subvolume.
...
our cosmology $r_s \approx 103\,\mathrm{Mpc}/h$.  That one length is
stamped into the matter distribution, and in Fourier space a single
preferred separation appears as a series of oscillations in $P(k)$ spaced
by
\begin{equation}
  \Delta k_{\rm BAO} \;=\; \frac{2\pi}{r_s}
  \;\approx\; 0.061 \; h\,\mathrm{Mpc}^{-1} .
\end{equation}
These are the \emph{baryon acoustic oscillations}, or BAO.  They are small:
in the linear spectrum used here they modulate $P(k)$ by about $1.8\%$ rms
over $k = 0.216$--$0.395\,h\,\mathrm{Mpc}^{-1}$.

It is useful to name the comparison spectrum too.  A \emph{de-wiggled}, or
no-wiggle, spectrum is $P(k)$ with the oscillations removed but the
broadband shape kept: the same slope, the same turnover, no ripples.  The
ratio of the true spectrum to the de-wiggled one isolates the BAO, and
oscillates about $1$.

\paragraph{Why a window damps them.}
...
$\Delta k \approx 2\pi/\ell$ in the matching direction.  This is the
uncertainty relation between a length and a wavenumber, and nothing more.
...
This also fixes the \emph{sign} of the effect at each $k$, which is what
makes it recognisable.  Averaging pulls every value toward the local mean.
At a wiggle peak the neighbours are lower, so the average sits below the
true value.  At a trough the neighbours are higher, so the average sits
above it.  The ratio of the convolved spectrum to the true one therefore
dips exactly where the true spectrum peaks and rises where it dips: the
ratio oscillates \emph{in antiphase} with the BAO.

\paragraph{A worked example: the pencil subvolume.}
Take the region used in the subvolume experiments of this repository: a
box of side $L = 700\,\mathrm{Mpc}/h$, and inside it a ``pencil'' occupying
one eighth of the box in two directions and its full length in the third,
so $87.5 \times 87.5 \times 700\,(\mathrm{Mpc}/h)^3$.  The transverse
extent gives
\begin{equation}
  \Delta k_\perp \;=\; \frac{2\pi}{87.5\,\mathrm{Mpc}/h}
  \;=\; 0.072 \; h\,\mathrm{Mpc}^{-1},
\end{equation}
which is slightly \emph{larger} than $\Delta k_{\rm BAO} = 0.061\,h\,
\mathrm{Mpc}^{-1}$.  Transversely, the averaging kernel is wider than one
full acoustic oscillation, so the prediction is that the wiggles are
heavily damped in such a region, not merely softened.

Two measurements confirm this.  First, the ratio of the convolved theory to
the raw theory oscillates in antiphase with the BAO, with a correlation
coefficient between $-0.63$ and $-0.70$ over
$k = 0.216$--$0.395\,h\,\mathrm{Mpc}^{-1}$, and with a matching amplitude
($1.4\%$ rms in the ratio against $1.8\%$ rms in the BAO).  Second, and
more directly, one can put a de-wiggled spectrum through the identical
convolution and see what is left.  The ripple in the ratio falls from
$3.13\%$ to $0.27\%$ rms over $k = 0.02$--$0.15\,h\,\mathrm{Mpc}^{-1}$,
and from $0.71\%$ to $0.02\%$ over $k = 0.216$--$0.395\,h\,
\mathrm{Mpc}^{-1}$.  Removing the wiggles from the input removes the
ripple from the output, so the ripple was the wiggles.
...
in that basis rather than by trying to undo the convolution.  The reason
is now visible in the wiggles specifically.  The convolved curve carries
damped BAO, and the raw curve carries full-amplitude BAO.  These are two
different curves, and the estimator is unbiased for the first one only.
Asking a measured subvolume spectrum to match the raw theory therefore asks
it to reproduce, in one realisation, acoustic wiggles at an amplitude that
its own ensemble average does not have.  A realisation can be found that
does so, by chance, but it has been chosen for a fluctuation rather than
for being typical, which is the subject of \texttt{seed\_selection.tex}.

Note finally that this has nothing to do with which species is used.  Under
the back-scaling convention that MUSIC2 and monofonIC use for initial
conditions, described in the companion note \texttt{cosmo\_ic.tex}, the
baryon and cold dark matter fields are both built from the same
total-matter field and differ
only by a smooth relative mode, so they carry the same acoustic wiggles to
within a few tenths of a percent.  Measured on the same subvolume, the
ripple amplitudes are $1.40\%$ for total matter, $1.39\%$ for cold dark
matter and $1.43\%$ for baryons.  Choosing the gas rather than the matter
does not avoid the damping.
