...
\title{The Grid Correlation Function:\\
       the Direct Lag Sum and the Padded FFT\\
...
\section{The question this note answers}\label{sec:question}

Suppose you want the two-point correlation function $\xi(r)$ of a density
field that has been sampled onto a grid inside some sub-region of a larger
periodic simulation box. There are two obvious ways to compute it.

The first is to do exactly what the definition says. For every separation
vector, walk over all pairs of cells separated by that vector, multiply the
two density contrasts, and average. This is a \emph{direct lag sum}. It is
transparently correct, and it is slow.

The second is to use the Fourier route. Transform the field, take the squared
modulus to get the power spectrum, and transform back. This is fast, but it
looks suspicious for at least three reasons. The Fourier transform is a
global operation, so information from the far side of the region seems to
leak into every output value. Taking $|\hat\delta|^2$ throws away all the
phase information, so it seems that something has been destroyed. And the
fast Fourier transform is a numerical algorithm with its own roundoff
behaviour, so one would expect it to give an \emph{approximation} to the
direct sum rather than the same answer.

Run both on the same data and they return the same number.

This note explains why that has to be so. The Fourier route is not a
numerical method that approximates the lag sum: it is an algebraic
rearrangement of the identical finite sum, so in exact arithmetic the two are
the same expression. The three objections above dissolve along the way, and
the only thing left over is why the last digits differ at all
(\S\ref{sec:roundoff}).

\subsection{How the note is arranged}

Sections~\ref{sec:setup}--\ref{sec:denominator} set up the object being
computed. They define the field and the mask (\S\ref{sec:setup}), show that
the grid estimator is the Landy--Szalay estimator rather than a simplified
stand-in for it (\S\ref{sec:ls}), and evaluate its denominator in closed form
(\S\ref{sec:denominator}). All of this is shared by both methods, which is
why it comes first.

Sections~\ref{sec:direct}--\ref{sec:roundoff} are the two methods and their
comparison: the direct sum (\S\ref{sec:direct}), the Fourier route with its
padding and per-axis boundary handling (\S\ref{sec:fft}), and the residual
floating-point difference (\S\ref{sec:roundoff}).

Sections~\ref{sec:cost}--\ref{sec:validates} are the practical part, and are
the sections to read if you are choosing between the two methods rather than
checking one against the other. They cover cost
(\S\ref{sec:cost}), what the Fourier route gives up
(\S\ref{sec:gives-up}), the bias from estimating the mean from the data
itself (\S\ref{sec:integral-constraint}), and what a two-method comparison
does and does not prove (\S\ref{sec:validates}).

\subsection{Relation to the literature}

None of the mathematics here is new. That the correlation function can be
obtained from a Fourier transform is old, and \citet{Slepian2016} give the
compact modern statement for galaxy surveys: the Landy--Szalay estimator is
\emph{itself} a binned convolution, so both its numerator and its denominator
can be evaluated with fast Fourier transforms in
$\mathcal{O}(N_g \log N_g)$ operations. Their \S2 is the direct reference for
\S\ref{sec:ls}, \S\ref{sec:pad-howmuch} and \S\ref{sec:gives-up} below, and
their paper goes considerably further than this note by extending the same
idea to the anisotropic two-point function and to the three-point function.

What this note adds is not new results but a slower path through the
two-point case, worked at the level of the individual array indices, for a
reader who wants to know why two programs on their disk return the same
number.

The concrete motivation is a pair of estimators in the KIAS
\texttt{HR-ICs-pipeline} repository, both under \texttt{tools/pk\_pipeline/}:
\texttt{measure\_xi\_mpi.c} evaluates the direct lag sum, and
\texttt{measure\_xi\_fft\_mpi.c} evaluates the padded FFT. They are
documented as agreeing bin-for-bin. Everything below is written so that it
stands on its own without those files.

\section{Setup: field, mask, and estimator}\label{sec:setup}
...
\subsection{What ``the estimator'' is}
...
\subsection{Uniform selection: the case at hand}\label{sec:ls-uniform}
...
generalises to the Fourier route without any assumption about the geometry.
...
\section{The pair count in closed form}\label{sec:denominator}

Both methods need $n(d) = \sum_x \W_x \W_{x+d}$, the number of valid pairs,
so we evaluate it once here before splitting into the two routes.
...
\section{Method A: the direct lag sum}\label{sec:direct}
...
\section{Method B: the Fourier route}\label{sec:fft}
...
\S\ref{sec:wk}. Let $f_x$ be an array of $P$ numbers indexed by
$x = 0,1,\dots,P-1$. Its discrete Fourier transform (DFT) is
\begin{equation}\label{eq:dft}
  \hat f_k \;=\; \sum_{x=0}^{P-1} f_x\, e^{-2\pi i k x / P},
  \qquad k = 0,1,\dots,P-1 ,
\end{equation}
and the inverse transform is
\begin{equation}\label{eq:idft}
  f_x \;=\; \frac{1}{P} \sum_{k=0}^{P-1} \hat f_k\, e^{+2\pi i k x / P} .
\end{equation}
The fast Fourier transform (FFT) is an algorithm for evaluating
Eq.~\eqref{eq:dft} in $\mathcal{O}(P\log P)$ operations instead of the
$\mathcal{O}(P^2)$ that the formula suggests \citep{CooleyTukey1965}. It
...
suspicion the Fourier route attracts. An FFT is a regrouping of the sum in
...
\begin{equation}\label{eq:orthogonality}
  \frac{1}{P}\sum_{k=0}^{P-1} e^{2\pi i k m / P}
  \;=\;
  \begin{cases}
    1 & \text{if } m \equiv 0 \pmod P,\\
    0 & \text{otherwise}.
  \end{cases}
\end{equation}
The proof is a geometric series and takes one line. Write
$z = e^{2\pi i m / P}$. If $m \equiv 0 \pmod P$ then $z = 1$ and the sum is
$P$, giving $1$ after the prefactor. Otherwise $z \neq 1$, and the finite
geometric series gives $\sum_{k=0}^{P-1} z^k = (z^P - 1)/(z - 1)$. The
numerator vanishes because $z^P = e^{2\pi i m} = 1$ for integer $m$, while
the denominator does not, so the sum is zero.

Note what Eq.~\eqref{eq:orthogonality} is: a Kronecker delta, but one that
identifies indices \emph{modulo} $P$. That ``modulo'' is the whole story of
...
\begin{equation}\label{eq:modsq}
  ...
  \;=\; \left(\sum_x f_x e^{-2\pi i k x / P}\right)
        \left(\sum_y f_y e^{+2\pi i k y / P}\right)
  \;=\; \sum_x \sum_y f_x f_y\, e^{-2\pi i k (x-y)/P} ,
\end{equation}
...
\begin{align}
  \frac{1}{P}\sum_k |\hat f_k|^2 e^{2\pi i k d / P}
  &= \sum_x \sum_y f_x f_y \;
     \underbrace{\frac{1}{P}\sum_k e^{2\pi i k\,(d - x + y)/P}}_{\text{use Eq.~\eqref{eq:orthogonality}}}
     \label{eq:wk-step1}\\[4pt]
  &= \sum_x \sum_y f_x f_y \;
     \big[\, x - y \equiv d \!\!\pmod P \,\big]
     \label{eq:wk-step2}\\[4pt]
  &= \sum_x f_x\, f_{(x-d) \bmod P} .
     \label{eq:wk-step3}
\end{align}
...
$x$, namely $y = (x-d) \bmod P$. Relabelling the dummy index turns this into
$\sum_x f_x f_{(x+d) \bmod P}$, which is the more familiar form.
...
identity between two finite sums of products of the same $P$ numbers, valid
...
index arithmetic is modulo $P$, so cell $P-1$ is treated as a neighbour of
...
Take $P = N = 4$ with $f = (a,b,c,d)$ and look at lag $1$. The circular
...
Now embed the $N$-cell array in a longer array of length $P$, filling the
...
top of the array with index $x + d - P$. For this to reach back into the
occupied region $[0,N)$ we need $x + d - P \ge 0$ with $x \le N-1$, that is
$d \ge P - N + 1$. So all lags with
...
$P = N + N/4$ suffices, and the transform is done on an array a quarter
...
Take the $P=4$ example above and pad to $P=8$, giving
...
\section{Rounding: why the agreement is not exact}\label{sec:roundoff}
...
is $2.2\times10^{-16}$ in double precision. A naive sequential sum of $M$
terms accumulates a relative error bounded by roughly $M\epsilon$, and in
practice, with errors of random sign, of order $\sqrt{M}\,\epsilon$. An FFT
of length $P$ performs $\log_2 P$ butterfly stages, and its error grows like
$\sqrt{\log_2 P}\,\epsilon$ \citep{Frigo2005}. For large arrays the FFT is
...
\section{Cost: when the Fourier route wins}\label{sec:cost}
...
The Fourier route costs $\mathcal{O}(N_g \log N_g)$ in the number of grid
...
\section{What the Fourier route gives up}\label{sec:gives-up}

The identity proved above is exact \emph{given the gridded field}. It says
nothing about the step before that, and it is there that the Fourier route
...
Fourier route \citep{Slepian2016}. Whatever was true of the pair-counted
monopole remains true of the transformed one.

\section{What a two-method test does and does not validate}\label{sec:validates}
...
\begin{enumerate}
  ...
  \item The FFT returns the \emph{circular} autocorrelation. Zero-padding a
        ...
        $d \le P-N$ (Eq.~\ref{eq:pad-condition}): pad by the largest
  ...
  \item Which method to \emph{run} is a question of cost, not correctness.
        The Fourier route wins for large separations, high number density and
        coarse grids; pair counting wins for small separations in sparse
        catalogues, and avoids gridding artefacts entirely.
  \item What the Fourier route concedes is all upstream of the identity:
