...
- `scripts/pipeline/make_music_conf.py` — generates a config from the template given N, z, L
...
### IC validation
- `scripts/pipeline/compute_pk.py` — estimates P(k) from IC particles via CIC + FFT; saves `pk_*.txt` (no plotting). Thin wrapper around `icpipe.ICField.power('delta')`.
- `scripts/pipeline/compute_pv.py` — estimates the velocity power spectrum P_v(k) via CIC velocity assignment + FFT; saves `pv_*.txt` (k[h/Mpc], Pv_raw[(km/s)²(Mpc/h)³], Pv_nodeconv, sigma_Pv, nmodes). Uses `icpipe.ICField.power('velocity')`.
...
# Manually — two steps:
conda run -n cosmo python scripts/pipeline/compute_pk.py data/ics_swift_n256_z200_L1024.hdf5
conda run -n cosmo python scripts/pipeline/plot_ic.py data/pk_n256_z200_L1024.txt \
