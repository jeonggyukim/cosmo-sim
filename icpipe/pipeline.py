"""End-to-end IC generation and validation pipeline.

Orchestrates two IC codes, selected by ``RunConfig.ic_code``:
  * ``music``     — legacy multi-scale MUSIC (zoom-capable; the default)
  * ``monofonic`` — MUSIC2-monofonIC (unigrid; adds PLT, 3LPT)

The IC front-end (build, config, run) branches on the code; every downstream
step (CLASS P(k), rbins, xi, psi, P(k), plotting) is code-agnostic because both
codes write the same SWIFT HDF5 output.

Steps (each skipped if its output already exists):
  1. Build the IC code
  2. Build compute_xi / compute_xi_cic (C, Corrfunc)
  3. Generate the IC config from a template
  4. Run the IC code -> SWIFT IC HDF5 (+ a CLASS ini)
  5. CLASS matter P(k) at z=0 (adapt the CLASS ini; reuse the shared
     data/class_pk_z0_pk.dat if already present)
  6. Corrfunc radial bin file
  7. Corrfunc xi(r)         (skipped at z > 10; shot-noise dominated)
  8. CIC-grid xi(r), psi(r) (any z)
  9. CIC + FFT P(k)
 10. Diagnostic plot

All outputs are keyed by STEM under ``RunConfig.root`` (default: the current
working directory):
  music     : n{N}_z{z}_L{L}[_nofix][_s{seed}]
  monofonic : n{N}_z{z}_L{L}_mono[_nofix][_s{seed}]
so MUSIC and monofonIC runs at the same N, z, L coexist without collision.

The heavy IC step can be launched under an MPI launcher (``launcher`` or
``mpi_ranks``) for multi-node / multi-rank runs on a cluster; the Python
orchestrator itself stays a single process — only the IC binary is launched
across ranks.
"""

from __future__ import annotations

import os
import resource
import shutil
import subprocess
import sys
import tempfile
from dataclasses import dataclass, field
from glob import glob

# CV_22 cosmology (Illustris/TNG) — matches both conf templates.
H0, OMEGA_M, OMEGA_B = "67.11", "0.3", "0.049"

# Shared, cosmology-keyed CLASS output (not stem-keyed): the theory P(k) depends
# only on cosmology, which both templates match, so a single file serves all runs.
CLASS_PKS = "data/class_pk_z0_pk.dat"


@dataclass
class RunConfig:
    """Parameters for one pipeline run.

    root : base directory for all inputs/outputs (conf/, data/, plots/, the
           build dirs and tools/); defaults to the current working directory.
    launcher : optional launcher prefix for the MPI IC step, e.g. "srun" or
           "srun -n 128". Split on whitespace and prepended to the IC binary.
    mpi_ranks : if > 1 and no launcher is set, run the IC binary under
           "mpirun -np {mpi_ranks}".
    """
    root: str = field(default_factory=os.getcwd)
    ic_code: str = "music"
    ngrid: str = "256"
    lbox: str = "1000"
    zstart: str = "200"
    nthreads: str = "4"
    fix_amplitude: str = "yes"
    lpt_order: str = "3"
    seed: int | None = None
    ic_source_dir: str | None = None
    corrfunc_dir: str | None = None
    launcher: str | None = None
    mpi_ranks: int = 1
    stop_after_ic: bool = False


# ---------------------------------------------------------------------------
# Small helpers
# ---------------------------------------------------------------------------

def log(msg: str) -> None:
    print(f"\n==> {msg}", flush=True)


def run(cfg: RunConfig, cmd: list[str], **kw) -> subprocess.CompletedProcess:
    """Run a command from ``cfg.root``, raising on failure."""
    return subprocess.run(cmd, cwd=cfg.root, check=True, **kw)


def py_cli(module: str, *args: str) -> list[str]:
    """Invoke an icpipe CLI through the current interpreter (no PATH/conda dependency)."""
    return [sys.executable, "-m", module, *map(str, args)]


def peak_child_mib() -> float:
    """Peak RSS of child processes so far, in MiB (ru_maxrss is bytes on macOS, kB on Linux)."""
    rss = resource.getrusage(resource.RUSAGE_CHILDREN).ru_maxrss
    div = 1048576 if sys.platform == "darwin" else 1024
    return rss / div


def move_if(cfg: RunConfig, src_rel: str, dst_rel: str) -> bool:
    """Move root/src_rel to root/dst_rel if the source exists; report and return success."""
    src = os.path.join(cfg.root, src_rel)
    if not os.path.exists(src):
        return False
    shutil.move(src, os.path.join(cfg.root, dst_rel))
    print(f"    Saved: {dst_rel}")
    return True


def _exists(cfg: RunConfig, rel: str) -> bool:
    return os.path.exists(os.path.join(cfg.root, rel))


# ---------------------------------------------------------------------------
# Per-code configuration
# ---------------------------------------------------------------------------

def stem_for(cfg: RunConfig) -> str:
    amp = "" if cfg.fix_amplitude == "yes" else "_nofix"
    sd = f"_s{cfg.seed}" if cfg.seed else ""
    mono = "_mono" if cfg.ic_code == "monofonic" else ""
    return f"n{cfg.ngrid}_z{cfg.zstart}_L{cfg.lbox}{mono}{amp}{sd}"


def ic_binary(ic_code: str) -> str:
    return "music_build/MUSIC" if ic_code == "music" else "monofonic_build/monofonIC"


def build_script(ic_code: str) -> str:
    return "tools/build-music.sh" if ic_code == "music" else "tools/build-monofonic.sh"


def conf_path(ic_code: str, stem: str) -> str:
    name = "CV_22_MUSIC" if ic_code == "music" else "CV_22_monofonIC"
    return f"conf/{name}_{stem}.conf"


def ic_launch_prefix(cfg: RunConfig) -> list[str]:
    """MPI launcher prefix for the IC binary (empty for a plain single-rank run)."""
    if cfg.launcher:
        return cfg.launcher.split()
    if cfg.mpi_ranks and cfg.mpi_ranks > 1:
        return ["mpirun", "-np", str(cfg.mpi_ranks)]
    return []


# ---------------------------------------------------------------------------
# Steps
# ---------------------------------------------------------------------------

def step_build_ic(cfg: RunConfig) -> None:
    binary = ic_binary(cfg.ic_code)
    if _exists(cfg, binary):
        log(f"{cfg.ic_code} already built — skipping")
        return
    log(f"Building {cfg.ic_code}...")
    env = dict(os.environ)
    if cfg.ic_source_dir:
        key = "MUSIC2_SOURCE_DIR" if cfg.ic_code == "music" else "MONOFONIC_SOURCE_DIR"
        env[key] = cfg.ic_source_dir
    run(cfg, [os.path.join(cfg.root, build_script(cfg.ic_code))], env=env)


def step_build_compute_xi(cfg: RunConfig) -> None:
    if _exists(cfg, "bin/compute_xi"):
        log("compute_xi already built — skipping")
        return
    log("Building compute_xi / compute_xi_cic...")
    cmd = ["make", "-C", "src"]
    if cfg.corrfunc_dir:
        cmd.append(f"CORRFUNCDIR={cfg.corrfunc_dir}")
    run(cfg, cmd)


def step_gen_config(cfg: RunConfig, stem: str) -> str:
    conf = conf_path(cfg.ic_code, stem)
    if _exists(cfg, conf):
        log(f"config already exists — skipping ({conf})")
        return conf
    log(f"Generating {cfg.ic_code} config...")
    if cfg.ic_code == "music":
        cmd = py_cli("icpipe.cli.make_music_conf",
                     "-N", cfg.ngrid, "-z", cfg.zstart, "-L", cfg.lbox,
                     "--fix-amplitude", cfg.fix_amplitude, "-o", os.path.join(cfg.root, conf))
    else:
        cmd = py_cli("icpipe.cli.make_monofonic_conf",
                     "-N", cfg.ngrid, "-z", cfg.zstart, "-L", cfg.lbox,
                     "--fixing", cfg.fix_amplitude, "--lpt-order", cfg.lpt_order,
                     "--nthreads", cfg.nthreads, "-o", os.path.join(cfg.root, conf))
    if cfg.seed:
        cmd += ["--seed", str(cfg.seed)]
    run(cfg, cmd)
    return conf


def step_run_ic(cfg: RunConfig, stem: str, conf: str) -> str:
    ic_file = f"data/ics_swift_{stem}.hdf5"
    if _exists(cfg, ic_file):
        log(f"IC file already exists — skipping ({ic_file})")
        return ic_file
    prefix = ic_launch_prefix(cfg)
    log(f"Running {cfg.ic_code}" + (f" ({' '.join(prefix)})" if prefix else "") + " ...")
    run(cfg, [*prefix, os.path.join(cfg.root, ic_binary(cfg.ic_code)), conf])
    mib = peak_child_mib()

    # Both codes write a CLASS ini to the CWD; normalize it to one path so the
    # CLASS step is code-agnostic.
    class_ini = f"conf/input_class_parameters_{stem}.ini"

    if cfg.ic_code == "music":
        # MUSIC writes input_class_parameters.ini and wnoise_*.bin to the CWD.
        move_if(cfg, "input_class_parameters.ini", class_ini)
        for f in sorted(glob(os.path.join(cfg.root, "wnoise_*.bin"))):
            move_if(cfg, os.path.basename(f), f"data/{os.path.basename(f)}")
    else:
        # monofonIC prefixes most aux outputs with the config basename (conf file
        # minus dir/ext) and writes them to the CWD (relative_to_config = no).
        base = os.path.splitext(os.path.basename(conf))[0]  # CV_22_monofonIC_{stem}
        move_if(cfg, f"{base}_input_class_parameters.ini", class_ini)
        for name in ("input_powerspec.txt", "input_transfer.txt", "log.txt"):
            move_if(cfg, f"{base}_{name}", f"data/{stem}_{name}")
        for f in sorted(glob(os.path.join(cfg.root, f"{base}_input_powerspec_sampled*.txt"))):
            suffix = os.path.basename(f)[len(base) + 1:]
            move_if(cfg, os.path.basename(f), f"data/{stem}_{suffix}")
        # GrowthFactors.txt is written with a fixed name (not config-prefixed).
        move_if(cfg, "GrowthFactors.txt", f"data/{stem}_GrowthFactors.txt")

    print(f"    Saved: {ic_file}")
    print(f"    Peak child memory: {mib:.1f} MiB")
    return ic_file


def find_class_bin(cfg: RunConfig) -> str | None:
    """Locate a standalone CLASS binary. Built by MUSIC's CMake; monofonIC links
    CLASS as a library and builds no standalone binary, so a monofonic-only setup
    relies on a CLASS binary (or a prebuilt data/class_pk_z0_pk.dat) from MUSIC."""
    for p in ("music_build/_deps/class-build/class",
              "monofonic_build/_deps/class-build/class"):
        if _exists(cfg, p):
            return p
    return None


def step_class(cfg: RunConfig, stem: str) -> str | None:
    if _exists(cfg, CLASS_PKS):
        log(f"CLASS P(k) already exists — skipping ({CLASS_PKS})")
        return CLASS_PKS

    # Both codes write a CLASS ini, normalized to this path in step_run_ic.
    class_ini = f"conf/input_class_parameters_{stem}.ini"
    class_bin = find_class_bin(cfg)
    if not _exists(cfg, class_ini) or class_bin is None:
        reason = ("no CLASS ini found" if not _exists(cfg, class_ini)
                  else "no standalone CLASS binary (build MUSIC once to get one)")
        log(f"Cannot build {CLASS_PKS}: {reason}. Theory overlay will be skipped.")
        return None

    log(f"Running CLASS at z=0 (back-scaled at plot time to z={cfg.zstart})...")
    fd, tmp = tempfile.mkstemp(suffix=".ini")
    os.close(fd)
    with open(os.path.join(cfg.root, class_ini)) as fin:
        lines = []
        for line in fin:
            s = line.strip()
            if s.startswith("output ="):
                line = "output = mPk\n"
            elif s.startswith("z_pk ="):
                line = "z_pk = 0\n"
            elif s.startswith("extra metric transfer functions") or s.startswith("gauge"):
                continue
            lines.append(line)
        lines.append("root = class_pk_z0_\n")
    with open(tmp, "w") as fout:
        fout.writelines(lines)
    run(cfg, [class_bin, tmp])
    shutil.move(os.path.join(cfg.root, "class_pk_z0_pk.dat"),
                os.path.join(cfg.root, CLASS_PKS))
    os.remove(tmp)
    print(f"    Saved: {CLASS_PKS}")
    return CLASS_PKS


def step_rbins(cfg: RunConfig, stem: str, ic_file: str) -> str:
    rbins = f"data/rbins_{stem}.txt"
    if _exists(cfg, rbins):
        log(f"rbins already exist — skipping ({rbins})")
        return rbins
    log("Generating rbins...")
    run(cfg, py_cli("icpipe.cli.make_rbins", "--hdf5", ic_file))
    return rbins


def step_xi(cfg: RunConfig, stem: str, ic_file: str, rbins: str) -> None:
    xi_file = f"data/xi_{stem}.txt"
    if float(cfg.zstart) > 10:
        log(f"Skipping Corrfunc xi(r) — z={cfg.zstart} > 10 (shot-noise dominated; use CIC)")
        return
    log(f"Measuring xi(r) with compute_xi ({cfg.nthreads} threads)...")
    with open(os.path.join(cfg.root, xi_file), "w") as out:
        run(cfg, ["./bin/compute_xi", ic_file, rbins, str(cfg.nthreads)], stdout=out)
    print(f"    Saved: {xi_file}")


def step_xi_cic(cfg: RunConfig, stem: str, ic_file: str) -> None:
    xi_cic = f"data/xi_cic_{stem}.txt"
    log(f"Measuring xi(r) and psi(r) on CIC grid ({cfg.nthreads} threads)...")
    run(cfg, ["./bin/compute_xi_cic", "--input", ic_file, "--Ngrid", "128",
              "--nthreads", str(cfg.nthreads), "--output", xi_cic, "--vel"])
    print(f"    Saved: {xi_cic}")
    print(f"    Saved: data/vel_cic_{stem}.txt")


def step_pk(cfg: RunConfig, stem: str, ic_file: str) -> str:
    pk_file = f"data/pk_{stem}.txt"
    log("Measuring P(k) with compute_pk...")
    run(cfg, py_cli("icpipe.cli.compute_pk", ic_file, "--H0", H0, "-o", pk_file))
    print(f"    Saved: {pk_file}")
    return pk_file


def step_plot(cfg: RunConfig, stem: str, pk_file: str, theory: str | None) -> str:
    plot_file = f"plots/pk_{stem}.png"
    log("Plotting diagnostics with plot_ic...")
    cmd = py_cli("icpipe.cli.plot_ic", pk_file,
                 "--H0", H0, "--Omega_m", OMEGA_M, "--Omega_b", OMEGA_B,
                 "-o", plot_file)
    if theory:
        cmd += ["--theory", theory, "--theory-zref", "0"]
    run(cfg, cmd)
    return plot_file


# ---------------------------------------------------------------------------
# Orchestration
# ---------------------------------------------------------------------------

def run_pipeline(cfg: RunConfig) -> None:
    """Run the full pipeline for ``cfg``."""
    for d in ("data", "plots", "conf"):
        os.makedirs(os.path.join(cfg.root, d), exist_ok=True)

    stem = stem_for(cfg)

    step_build_ic(cfg)
    step_build_compute_xi(cfg)
    conf = step_gen_config(cfg, stem)
    ic_file = step_run_ic(cfg, stem, conf)

    if cfg.stop_after_ic:
        log("--stop-after-ic set: exiting before CLASS P(k).")
        return

    theory = step_class(cfg, stem)
    rbins = step_rbins(cfg, stem, ic_file)
    step_xi(cfg, stem, ic_file, rbins)
    step_xi_cic(cfg, stem, ic_file)
    pk_file = step_pk(cfg, stem, ic_file)
    plot_file = step_plot(cfg, stem, pk_file, theory)

    log("Pipeline complete.")
    print(f"    IC code : {cfg.ic_code}")
    print(f"    IC file : {ic_file}")
    print(f"    P(k)    : {pk_file}")
    print(f"    Plot    : {plot_file}")
